...
public class SockCollector : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        // Automatically deliver the item when touching Laundry
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        // Clear the current collider when exiting
        if (collision == currentCollider)
        {
            currentCollider = null;
        }
    ...
    private void DeliverItem()
    {
        ...
        {
            // Delivering a non-BlueSock item restarts the scene
            RestartScene();
        }
    }
    ...
    private void FixedUpdate()
    {
        // Make the current item follow the Player if it's assigned
